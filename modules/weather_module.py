...
def fetch_and_cache_weather():
    ...
    try:
        ...
        _WEATHER_CACHE["air_quality"] = aq_data.get("list", [{}])[0].get("main", {}).get("aqi")
    except Exception:
...
def get_weather():
    now = time.time()
    if "onecall" not in _WEATHER_CACHE or now - _WEATHER_CACHE.get("onecall_timestamp", 0) > _TTL:
        fetch_and_cache_weather()
    data = _WEATHER_CACHE["onecall"]
    current = data.get("current", {})
    weather = current.get("weather", [{}])[0]
    moon_phase = data.get("daily", [{}])[0].get("moon_phase") if data.get("daily") else None
    _WEATHER_CACHE["data"] = {
        "temp": c_to_f(current.get("temp")),
        "icon": weather.get("icon"),
        "description": weather.get("description"),
        "low": c_to_f(data.get("daily", [{}])[0].get("temp", {}).get("min")) if data.get("daily") else None,
        "high": c_to_f(data.get("daily", [{}])[0].get("temp", {}).get("max")) if data.get("daily") else None,
